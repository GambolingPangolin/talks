...
import           Polysemy.Trace (Trace (..), ignoreTrace, trace)
...
traceToFile :: Member (Embed IO) r => FilePath -> Sem (Trace ': r) a -> Sem r a
traceToFile file = interpret $ \case
    Trace x -> embed . appendFile file $ x <> "\n"


main :: IO ()
main = runM . traceToFile "/tmp/game-2.log" $ runGameIO game
...
game = do
    ...
    trace "Generating random number"
    randomR 1 100 >>= loop
